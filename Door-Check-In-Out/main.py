...
capture = cv2.VideoCapture("../videos/dataset1.mp4")
...
while capture.isOpened():
    ...
    results = model(frame, conf=0.4, iou=0.25)[0]
    detections = []
    ...
    for track in tracks:
        ...
        cv2.circle(frame, (cx, cy), 4, (0, 255, 255), -1)
        cv2.arrowedLine(frame, (prev_cx, prev_cy), (cx, cy), (255, 0, 255), 2)
        # Show IN/OUT status on each track (optional, for clarity)
        status = "IN" if current_in else "OUT"
        cv2.putText(frame, f"{status}", (x1, y2 + 15),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 255, 0) if current_in else (0, 128, 255), 1)
