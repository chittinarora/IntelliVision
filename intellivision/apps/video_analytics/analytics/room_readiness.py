...
def tracking_video(self, video_path: str, output_dir: str = str(OUTPUT_DIR)) -> Dict:
    ...
    logger.info(f"**Job {job_id}**: Progress **100.0%** ({result['meta'].get('frames_analyzed', 1)}/{result['meta'].get('frames_analyzed', 1)}), Status: {result['status']}...")
    logger.info(f"[##########] Done: {int(processing_time // 60):02d}:{int(processing_time % 60):02d} | Left: 00:00 | Avg FPS: N/A")
    return result
